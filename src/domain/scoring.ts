...
export function signalsFromSnapshot(stockId: StockId, snapshot: MarketSnapshot, holding?: Holding): Signal[] {
  return getBaseSignals(stockId, holding).map((signal) => mapGenericSignal(signal, stockId, snapshot, holding));
}
...
function buildProfileFromSignals(holding: Holding, signals: Signal[]): StockProfile {
  ...
  const status = deriveStatus(score, redSignals);
  ...
  return {
    ...
    reasons,
    nextWatch: buildNextWatch(holding)
  };
}

function deriveStatus(score: number, redSignals: Signal[]): StockStatus {
  const redIds = new Set(redSignals.map((signal) => signal.id));

  if (redIds.has("price-action") && redIds.has("liquidity")) return "red";
  if (redIds.has("liquidity") && score > 15) return "overheated";
...
function buildNextWatch(holding: Holding): string[] {
  return [
